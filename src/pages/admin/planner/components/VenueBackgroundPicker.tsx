...
export default function VenueBackgroundPicker({ venueBg, setVenueBg, showBgPicker, setShowBgPicker }: VenueBackgroundPickerProps) {
  ...
  return (
    <div className="fixed left-3 right-3 top-3 z-[9999] sm:absolute sm:left-auto sm:right-0 sm:top-full sm:mt-1 sm:z-50 bg-white rounded-lg shadow-xl border border-slate-200 p-2 sm:w-56 max-h-[70vh] overflow-y-auto">
      <p className="text-[10px] font-bold text-slate-500 uppercase mb-1.5 px-1">Interior</p>
      <div className="grid grid-cols-3 sm:grid-cols-3 gap-1 mb-2">
        {VENUE_BACKGROUNDS.filter((b) => b.group === "Interior").map((b) => {
          const Icon = b.icon;
          return (
            <button key={b.id} onClick={() => { setVenueBg(b.id); setShowBgPicker(false); }}
              className={`flex flex-col items-center gap-0.5 p-2 sm:p-1.5 rounded text-[9px] transition-colors ${venueBg === b.id ? "bg-amber-100 text-amber-700 border border-amber-300" : "hover:bg-slate-50 text-slate-600 border border-transparent"}`}>
